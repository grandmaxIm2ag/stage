...
$$
We can see $K$-Means algorothm\ref{algo:kmeans}
\begin{algorithm}
  ...
  \Repeat{Convergence}{
    ...
    \ForEach{centroids $r_k$}{
      $r_k^t \gets \frac{1}{|S_k|}\sum_{X \in S_k}X$.\\
    }
    $t++$\\
  }
  \Return{S}
...
\begin{figure}[!h]
  \centering
  \fbox{\includegraphics[scale=0.4]{parts/res/autoencoder.png}}
...
\right.
\end{equation}
where $\beta$ play the role of an inverse temperature, and 
